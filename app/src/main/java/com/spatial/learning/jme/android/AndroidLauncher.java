...
public class AndroidLauncher extends AppCompatActivity implements TouchListener, DialogInterface.OnClickListener, SystemListener {
     ...
     */
    protected String appClass = SpatialLearningVWM.class.getCanonicalName();
    /**
    ...
    public void startGame(String playerName, String filePath) {
    ...
    public void handleError(final String errorMsg, final Throwable t) {
        ...
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                AlertDialog dialog = new AlertDialog.Builder(AndroidLauncher.this) // .setIcon(R.drawable.alert_dialog_icon)
                        .setTitle(finalTitle).setPositiveButton("Kill", AndroidLauncher.this).setMessage(finalMsg).create();
                dialog.show();
            }
        });
    }
            ...
            switch (evt.getType()) {
                case KEY_UP:
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            AlertDialog dialog = new AlertDialog.Builder(AndroidLauncher.this) // .setIcon(R.drawable.alert_dialog_icon)
                                    .setTitle(exitDialogTitle).setPositiveButton("Yes", AndroidLauncher.this).setNegativeButton("No", AndroidLauncher.this).setMessage(exitDialogMessage).create();
                            dialog.show();
                        }
                    });
                    break;
